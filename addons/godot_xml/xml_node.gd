...
func to_dict() -> Dictionary:
	...
	for child in self.children:
		if child.name == "li":
			...
			index += 1
		children_dict.merge({child.name: child.to_dict()})

	output["children"] = children_dict

	return output

#children tags are in array
func to_dict_cia(always: bool = true) -> Dictionary:
	var output := {}
	...
	for child in self.children:
		if child.name == "li":
			...
			index += 1
		if always:
			children_dict.append({child.name: child.to_dict_cia()})
		else:
			children_dict.append({child.name: child.to_dict()})
